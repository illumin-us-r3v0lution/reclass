...
class ConfigBase(UserDict.UserDict):
    ...
    def __init__(self, opts=None, rt={}, lgr=None, files=[], *args, **kwargs):
        '''
        '''
    ...
    def _load_files(self):
        ...
        is parsed as either YAML or JSON

        '''
        data = Parameters({})
        for config in self._filelist:
            data.merge(self._load_file(config))
        return data.as_dict()
    ...
    def _load_file(self, fpath):
        ...
        # return an empty dict if we are unable to load the file
        data = {}
    ...
    def save(self, fpath=None):
        ...
        try:
            with open(self.file_path, 'w') as config:
                msg = ('writing %s file to %s' % (self.file_format, self.file_path))
                self.logger.debug(msg)
